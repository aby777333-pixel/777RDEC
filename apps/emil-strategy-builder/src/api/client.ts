...
/**
 * Strategy-lab backend (dashboard, strategies, positions, audit log…). On the
 * 777 Raptor site there is none, so unless VITE_API_BASE is set at build time
 * every call below resolves straight to its demo data instead of polling
 * endpoints that do not exist.
 */
export const API_BASE: string = ENV.VITE_API_BASE || '';
export const HAS_LAB_BACKEND = API_BASE !== '';

/** Push channel for live agent events. Only opened when VITE_WS_URL is set. */
export const WS_URL: string = ENV.VITE_WS_URL || '';

/**
 * Raptor Market API (real quotes, candles, search, feed health). Served on the
 * host site by the shared read-only proxy at /api/raptor-market, which every
 * EMIL app uses and which forwards to the market API's own origin.
 */
export const MARKET_API: string = ENV.VITE_MARKET_API_BASE || '/api/raptor-market';

const TIMEOUT = 6000;

async function req<T>(path: string, init?: RequestInit): Promise<T> {
  if (!HAS_LAB_BACKEND) throw new Error('No strategy-lab backend configured');
...
/** GET against the Raptor Market API proxy. Independent of the lab backend. */
async function marketReq<T>(path: string): Promise<T> {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), TIMEOUT);
  try {
    const res = await fetch(`${MARKET_API}${path}`, { signal: ctrl.signal });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return (await res.json()) as T;
  } finally {
    clearTimeout(timer);
