...
const axis = { stroke: '#979da6', fontSize: 10 };
const grid = '#262d38';
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={180}>
        <AreaChart data={chartData}>
          ...
          <Area type="monotone" dataKey="drawdown" stroke="#f87171" fill="url(#dd)" strokeWidth={1.5} dot={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
export function MonthlyHeatmap({ equity }: { equity: EquityPoint[] }) {
  ...
  const bg = (v: number | null) => {
    if (v == null) return 'rgba(38,45,56,0.3)';
    const abs = Math.min(Math.abs(v) / 8, 1);
    return v >= 0 ? `rgba(52,211,153,${0.15 + abs * 0.6})` : `rgba(248,113,113,${0.15 + abs * 0.6})`;
  };
