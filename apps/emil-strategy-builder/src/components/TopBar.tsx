...
export function TopBar() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const r = await fetch(`${MARKET_API}/market/health`);
  ...
  return (
    <header className="h-14 shrink-0 flex items-center justify-between gap-2 px-3 sm:px-4 border-b border-border bg-card/60 backdrop-blur-md z-30">
      <div className="flex items-center gap-2 sm:gap-3">
        <a
          ...
          title={`Back to ${APP_TITLE} on ${BRAND.company}`}
        >
          <ArrowLeft size={14} /> <span className="hidden sm:inline">{BRAND.family}</span>
        </a>
        <span className="text-base sm:text-lg lg:text-xl font-extrabold tracking-tight whitespace-nowrap">
          <span className="text-primary">{BRAND.family}</span>{' '}
      ...
      <div className="flex items-center gap-1.5 sm:gap-3">
        ...
        <div
          ...
          title={
            feed.status === 'live'
              ? `${BRAND.platform} Market API live —${feed.count} data providers active (Finnhub, EODHD, Yahoo, Binance…). Real quotes over REST.`
              : feed.status === 'checking'
