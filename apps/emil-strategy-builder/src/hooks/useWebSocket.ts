...
export function useWebSocket() {
  ...
  useEffect(() => {
    // No push backend configured: stay disconnected, and the pipeline runs as
    // the client-side simulation (see simulatePipeline) instead of retrying a
    // socket that can never open.
    if (!WS_URL) return;
