...
const LEVEL_COLOR: Record<string, string> = {
  Trade: '#7dd3fc', Account: '#facc15', Portfolio: '#a78bfa', Strategy: '#34d399',
};
          ...
          <div className="flex flex-col gap-1.5 mb-4">
            {KILL_ACTIONS.map((a) => (
              <label key={a} className="flex items-center gap-2 text-xs cursor-pointer">
                <input type="checkbox" checked={!!actions[a]} onChange={() => toggle(a)} className="accent-[#f87171]" />
                {a}
              </label>
            ))}
          </div>
...
function RiskBar({ label, value, cap, warn }: { label: string; value: number; cap: string; warn?: boolean }) {
  const color = value >= 80 ? '#f87171' : warn || value >= 60 ? '#facc15' : '#34d399';
  return (
    <div>
