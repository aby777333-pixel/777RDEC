...
import { api, HAS_LAB_BACKEND, MARKET_API } from '../api/client';
...
const CONNECT_TARGETS: Record<ConnKind, string[]> = {
  broker: [`${BRAND.platform} Terminal`, 'MetaTrader 5', 'Interactive Brokers', 'cTrader', 'OANDA', 'Zerodha', 'TradeStation'],
...
function ApiKeys() {
  ...
  // Key issuance lives on the Raptor backend. Without one configured, show how
  // keys are obtained instead of calling endpoints this host does not serve.
  const base = HAS_LAB_BACKEND && typeof window !== 'undefined' ? window.location.origin : 'https://<your-raptor-host>';

  const load = useCallback(async () => {
    if (!HAS_LAB_BACKEND) { setLoading(false); return; }
    setLoading(true);
    try {
  ...
  return (
    <Card>
      <SectionTitle right={<KeyRound size={14} className="text-primary" />}>{BRAND.platform} API Keys</SectionTitle>
      ...
      {HAS_LAB_BACKEND ? (
        ...
        <div className="glass !rounded-lg p-3 mb-3 border border-primary/30 text-xs text-subtext">
          Keys are issued per account from the {BRAND.platform} terminal, not from this demo.{' '}
          <a href={BRAND.sandboxHref} className="text-primary hover:underline">Request sandbox access</a>{' '}
          to get keys against simulated data.
        </div>
      )}
...
function RaptorEcosystem() {
  return (
    <Card>
      <SectionTitle right={<Network size={14} className="text-primary" />}>{BRAND.platform} Ecosystem</SectionTitle>
      <p className="text-[11px] text-subtext mb-2">
        The builder maps into the {BRAND.company} stack. Tech Hub changes (symbols, spreads, markups, execution
...
function DataFeeds() {
  ...
  useEffect(() => {
    let alive = true;
    fetch(`${MARKET_API}/market/health`)
  ...
  return (
    <Card>
      ...
      {!loaded ? (
        <p className="text-xs text-subtext">Querying the {BRAND.platform} Market API…</p>
      ) : (
      ...
      <p className="text-[10px] text-subtext mt-2">Live from <span className="font-mono">/api/market/health</span> — the unified {BRAND.platform} Market API.</p>
    </Card>
  );
}
    ...
    <Card>
      ...
      <div className="flex flex-col gap-2">
        ...
        <label className="flex items-center gap-2 text-[11px] cursor-pointer text-subtext">
          <input type="checkbox" checked={noWithdraw} onChange={(e) => setNoWithdraw(e.target.checked)} className="accent-[#34d399]" />
