...
const REGIMES = [
  { label: 'Trending', color: '#34d399' }, { label: 'Ranging', color: '#facc15' },
  { label: 'Volatile', color: '#f87171' }, { label: 'Risk-off', color: '#a78bfa' },
  { label: 'Trending', color: '#34d399' }, { label: 'Crisis', color: '#ff2222' },
          ...
          <div style={{ height: 240 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monteCarlo.data}>
                ...
                <XAxis dataKey="i" tick={{ fill: '#979da6', fontSize: 10 }} />
                <YAxis tick={{ fill: '#979da6', fontSize: 10 }} width={48} domain={['auto', 'auto']} />
                {Array.from({ length: monteCarlo.paths }, (_, p) => (
                  <Line key={p} type="monotone" dataKey={`p${p}`} stroke="#7dd3fc" strokeWidth={1} dot={false} strokeOpacity={0.28} isAnimationActive={false} />
                ))}
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div style={{ height: 240 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={walkForward}>
                <CartesianGrid stroke="#262d38" strokeDasharray="3 3" />
                <XAxis dataKey="window" tick={{ fill: '#979da6', fontSize: 10 }} />
                <YAxis tick={{ fill: '#979da6', fontSize: 10 }} width={30} />
                <Tooltip contentStyle={{ background: '#0e1218', border: '1px solid #262d38', borderRadius: 8, fontSize: 12 }} />
                <Bar dataKey="in_sample" fill="#7dd3fc" name="In-sample Sharpe" />
                <Bar dataKey="out_sample" fill="#34d399" name="Out-of-sample Sharpe" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div style={{ height: 220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={sensitivity}>
                <CartesianGrid stroke="#262d38" strokeDasharray="3 3" />
                <XAxis dataKey="period" tick={{ fill: '#979da6', fontSize: 10 }} label={{ value: 'MA period', fill: '#979da6', fontSize: 10, position: 'insideBottom', dy: 10 }} />
                <YAxis tick={{ fill: '#979da6', fontSize: 10 }} width={30} />
                <Tooltip contentStyle={{ background: '#0e1218', border: '1px solid #262d38', borderRadius: 8, fontSize: 12 }} />
          ...
          <div style={{ height: 220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={radar} outerRadius="72%">
                <PolarGrid stroke="#262d38" />
                <PolarAngleAxis dataKey="k" tick={{ fill: '#94a3b8', fontSize: 9 }} />
                <PolarRadiusAxis domain={[0, 100]} tick={false} axisLine={false} />
                <Radar dataKey="v" stroke="#7dd3fc" fill="#7dd3fc" fillOpacity={0.35} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        </Card>
          ...
          <div style={{ height: 200 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={exposure} layout="vertical">
                <CartesianGrid stroke="#262d38" strokeDasharray="3 3" />
                <XAxis type="number" tick={{ fill: '#979da6', fontSize: 10 }} unit="%" />
                <YAxis type="category" dataKey="asset" tick={{ fill: '#94a3b8', fontSize: 10 }} width={54} />
                <Tooltip contentStyle={{ background: '#0e1218', border: '1px solid #262d38', borderRadius: 8, fontSize: 12 }} />
                <Bar dataKey="exposure" radius={[0, 4, 4, 0]}>
                  {exposure.map((e, i) => <Cell key={i} fill={e.exposure > 25 ? '#f87171' : e.exposure > 15 ? '#facc15' : '#7dd3fc'} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
              ...
              {corr.map((row, i) => (
                <tr key={i}>
                  ...
                  {row.map((v, j) => {
                    ...
                    const bg = pos ? `rgba(125,211,252,${alpha * 0.7})` : `rgba(248,113,113,${alpha * 0.7})`;
