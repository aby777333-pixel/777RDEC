...
import { MARKET_API } from '../api/client';
...
export default function Explore() {
  ...
  useEffect(() => {
    ...
    debounce.current = setTimeout(async () => {
      try {
        const r = await fetch(`${MARKET_API}/market/search?q=${encodeURIComponent(q.trim())}`);
  ...
  const loadSymbol = async (symbol: string, meta?: Result) => {
    ...
    try {
      const [qr, cr] = await Promise.all([
        fetch(`${MARKET_API}/market/quote?symbol=${encodeURIComponent(symbol)}`).then((r) => r.ok ? r.json() : null),
        fetch(`${MARKET_API}/market/candles?symbol=${encodeURIComponent(symbol)}&tf=${tf}&bars=120`).then((r) => r.ok ? r.json() : null),
      ]);
  ...
  useEffect(() => {
    if (!sel) return;
    let alive = true;
    fetch(`${MARKET_API}/market/candles?symbol=${encodeURIComponent(sel.symbol)}&tf=${tf}&bars=120`)
                  ...
                  {chartData.length > 1 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={chartData}>
                        ...
                        <XAxis dataKey="i" tick={false} axisLine={{ stroke: '#262d38' }} />
                        <YAxis domain={['auto', 'auto']} tick={{ fill: '#979da6', fontSize: 10 }} width={56} />
                        ...
                        <Area type="monotone" dataKey="close" stroke={up ? '#34d399' : '#f87171'} fill="url(#exQ)" strokeWidth={2} />
                      </AreaChart>
                    </ResponsiveContainer>
                  ) : (
          ...
          <Card>
            ...
            {candles.length > 0 && (
              <div className="mt-4 grid grid-cols-2 gap-2">
                ...
                <Stat label="High" value={fmtMoney(Math.max(...candles.slice(-1).map(c => c.high)), digits)} color="#34d399" />
                <Stat label="Low" value={fmtMoney(Math.min(...candles.slice(-1).map(c => c.low)), digits)} color="#f87171" />
                <Stat label="Prev Close" value={quote ? fmtMoney(quote.prev_close, digits) : '—'} />
              </div>
            )}
            <p className="text-[10px] text-subtext mt-3">
              Data via the {BRAND.platform} Market API. Analysis is simulated (demo) — no live orders.
            </p>
          </Card>
        </div>
      )}
