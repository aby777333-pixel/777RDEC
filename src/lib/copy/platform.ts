...
export const platformHub: PageCopy = {
  ...
  modules: [
    ...
    { title: EMIL_SHORT, body: `${EMIL_EXPANSION}. Observes, learns from live outcomes, protects the capital boundaries you set, and takes trades only once you have armed it.` },
...
export const terminal: PageCopy = {
  ...
  answers: {
    ...
    connects: `Fills flow straight to the journal and execution analytics. The risk engine validates every ticket before it leaves the browser and again on the server. ${EMIL_SHORT} observes the same surface and, when armed, acts through the same gate.`,
...
export const emil: PageCopy = {
  ...
  answers: {
    ...
    why: 'Markets change, strategies decay, relationships shift and risk moves. A fixed rule set is a snapshot of a market that has already gone — and a system that only adapts when someone retunes it is stale for most of its life.',
    ...
    next: `Read how it re-ranks its own inputs, what it is permitted to do once armed, and how the capital architecture keeps protected capital out of reach. Then bring your own risk policy to a demo.`,
  },
  modules: [
    ...
    { title: 'Understand', body: 'Regime classification — trending, ranging, expanding, contracting — stated as a reading, not a prediction.' },
    { title: 'Learn', body: 'Inputs are scored against what subsequently happened and re-ranked continuously. A relationship that stops holding stops driving the reading, without anyone retuning it.' },
    { title: 'Protect', body: 'Protected capital, profit floor, drawdown guard from the high-water mark, daily loss budget, exposure and concentration limits — all enforced before an order exists.' },
    { title: 'Act', body: 'Armed, it opens, closes, modifies and hedges — only with the permissions you granted, only in the markets you selected, only while armed.' },
    { title: 'Explain', body: 'Every event is logged in readable language, refusals included, with the inputs that produced it.' },
  ],
  ctaHeading: `Bring your risk policy`,
  ctaBody: `The useful version of this conversation is the one where we configure ${EMIL_SHORT}'s mandate against limits you actually enforce, then breach one deliberately and read the refusal together.`,
  ctaActions: [
    { label: `Open ${EMIL_SHORT} Lab`, href: '/intelligence/emil-lab', variant: 'primary' },
    { label: 'Request a demo', href: '/request-demo', variant: 'ghost' },
  ],
...
export const risk: PageCopy = {
  ...
  answers: {
    ...
    connects: `Every order — human or automated — passes the same checks. The engine is what bounds ${EMIL_SHORT}: the intelligence layer can conclude anything it likes and still be refused here, with the reason logged.`,
  ...
  modules: [
    ...
    { title: 'Loss and drawdown guards', body: 'Daily loss budgets and drawdown-from-high-water-mark limits, with a defined action on breach — stop opening, stop acting, or disarm automation entirely.' },
    { title: 'Protected capital', body: 'Declare a portion untouchable and set a profit floor that ratchets up as gains are banked. Automation cannot draw on either.' },
