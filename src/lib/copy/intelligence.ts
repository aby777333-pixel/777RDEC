...
export const intelligenceHub: PageCopy = {
  ...
  modules: [
    ...
    { title: EMIL_SHORT, body: 'Self-adjusting intelligence inside the platform. Learns from live outcomes, protects ring-fenced capital, and takes trades only once armed.' },
...
export const riskIntelligence: PageCopy = {
  ...
  answers: {
    ...
    next: 'Look at the risk views against a book with real shape to it, rather than a tidy example.',
  },
  modules: [
  ...
  ctaBody: 'The risk views are the part of a demo most people spend longest on.',
  ctaActions: [
...
export const emilLab: PageCopy = {
  ...
  lead: `${EMIL_SHORT} Lab is where a mandate is written: which inputs it may look at, which markets it may touch, how much capital is at risk, and how far it may go without you. Arming requires reading the authorisation text and typing a confirmation. Disarming is one button, always visible, no confirmation.`,
  answers: {
    what: `The control surface for ${EMIL_SHORT}: analysis permissions, market selection, protected capital and profit floor, drawdown and daily loss guards, maximum exposure, automation level, and the individual trading permissions — open, close, modify, hedge.`,
    who: 'Anyone deciding whether automation that adapts on its own is something they would be willing to authorise, and on what terms.',
    ...
    connects: 'In the product the same control surface is wired to the live position book, and every action it proposes passes the risk engine.',
    next: 'Grant close without open, so it may reduce risk but never create it. Many desks run exactly that way for months before granting anything more.',
  },
  modules: [
    { title: 'Analysis permissions', body: 'Which inputs it may look at: price and spread, volatility, cross-asset correlation, account exposure, session behaviour.' },
    { title: 'Markets', body: 'It may not act outside the instruments you select. A request for anything else is declined and logged.' },
    { title: 'Protected capital', body: 'A portion declared untouchable, plus a profit floor that ratchets up as gains are banked and never down.' },
    { title: 'Risk limits', body: 'Maximum drawdown from the high-water mark, daily loss budget, aggregate and per-instrument exposure ceilings.' },
    { title: 'Automation level', body: 'Analyse only, Suggest, or Execute within mandate. The level decides what is possible at all.' },
    { title: 'Trading permissions', body: 'Open, close, modify and hedge are granted individually. Close without open lets it reduce risk but never create it.' },
  ],
